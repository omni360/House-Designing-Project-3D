...
BOOL CEnBitmap::Attach(HICON hIcon, COLORREF crBack)
{
	ASSERT(m_hObject == NULL);      // only attach once, detach on destroy
	...
	if (dcMem.CreateCompatibleDC(pDC))
	{
		...
		if (bmMem.CreateCompatibleBitmap(pDC, 32, 32))
		{
			CBitmap* pOldBM = dcMem.SelectObject(&bmMem);

			if (crBack != -1)
				dcMem.FillSolidRect(0, 0, 32, 32, crBack);
	...
	BOOL CEnBitmap::Attach(HGDIOBJ jObject)
{
	CBitmap::Attach(jObject);
	return true;
}
