...
IMPLEMENT_SERIAL(CWall,CBuildingElement,1)
...
void CWall::Draw(CDC *pDC, BOOL bSelected)
{
	CPen   pen, *oldpen;
	LOGPEN p;
...
BOOL CWall::HitTest(CPoint ptHit)
{
	int Tol=m_uiWidth;
...
CWall::CWall(CPoint p1, CPoint p2)
{
...
CWall::CWall(CWall& w)
{
	m_ptFirstPosition = w.m_ptFirstPosition;
	m_ptSecondPosition= w.m_ptSecondPosition;
    
	//m_ptdoortop=m_stwall.SetValue().m_ptdoortop;
	
	//m_ptdoorbottom=m_stwall.SetValue().m_ptdoorbottom;;

	m_SideAColor = w.m_SideAColor;
	m_SideBColor = w.m_SideBColor;
//	m_SideATexture = w.m_SideATexture;
//	m_SideBTexture = w.m_SideBTexture;
	m_uiHeight=w.m_uiHeight;
	m_uiWidth=w.m_uiWidth;
	m_IsTexturizedSideA=FALSE;
	m_IsTexturizedSideB=FALSE;
	m_isdoor=FALSE;
	m_iswindow=FALSE;
...
void CWall::AddFixture(CFixture *pFix)
{
	m_FixtureList.Add(pFix);
}

void CWall::RemoveFixture(CFixture *pFix)
{
	//m_FixtureList
}

UINT CWall::FixtureCount() const
{
	return (m_FixtureList.GetSize());
}

void CWall::Serialize(CArchive &ar)
{
...
UINT CWall::GetType()
{
	return BUILDING_ELEMENT_WALL;
}


int CWall::calculate2(CWall w)
{
	diff=(int)((w.m_ptSecondPosition.x-w.m_ptFirstPosition.x)/58);
	return diff;
}
